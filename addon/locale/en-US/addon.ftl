...
service-customllm-dialog-model-hint=Use the exact model ID of your provider, for example deepseek-flash or deepseek-v4-pro (DeepSeek), glm-5.3-flash or kimi-k2.6 (OpenCode). Available IDs change over time.
service-customllm-dialog-thinking-hint=Tip: DeepSeek and several OpenCode models think before answering, which is slow for translation. In Custom Request add a field named thinking with the value {"type": "disabled"} to turn it off.
...
service-customllm-test-ok-empty=OK - { $model } answered in { $ms } ms, but sent only reasoning text ({ $tokens } reasoning tokens, finish reason: { $reason }). The Base URL, the API key and the model ID are all correct. This model thinks before answering: disable thinking mode in Custom Request to make translation much faster.
service-customllm-test-ok-empty=OK - { $model } answered in { $ms } ms, but sent only reasoning text ({ $tokens } reasoning tokens, finish reason: { $reason }). The Base URL, the API key and the model are correct. This model thinks before answering: disable thinking mode in Custom Request to make translation much faster.
...
service-customllm-error-empty-reasoning=The model spent the whole output budget on thinking ({ $tokens } reasoning tokens, finish reason: { $reason }) and produced no translation. Disable thinking mode in Custom Request - for DeepSeek add a thinking object with type disabled - or raise the max output tokens.
service-customllm-error-empty-truncated=The answer stopped before any text was produced (finish reason: length). Raise the max output tokens.
service-customllm-error-empty-reasoning=The model spent the whole output budget on thinking ({ $tokens } reasoning tokens, finish reason: { $reason }) and produced no translation. Disable thinking mode in Custom Request - for DeepSeek add {"thinking": {"type": "disabled"}} - or raise the max output tokens.
service-customllm-error-empty-truncated=The answer was cut off before any text was produced (finish reason: length). Raise the max output tokens.
